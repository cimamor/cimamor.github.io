...
{
	$('#seleccionNotaPrincipal').append('<div id="nota_'+ guid +'" class="notaPrincipal" style="background-color:'+ color +';"><section name="title" class="title"><input id="closeNotaId_'+ guid +'" type="image" src="img/icono_close.png" alt="Cerrar Nota" class="inputCloseNota" role="button"/><h1>'+ titulo +'</h1></section><section name="message" class="message"><p>'+ mensaje +'</p></section><input type="image" src="img/icono_delete.png" alt="Eliminar Nota" class="inputDeleteNota" id="deleteNotaId_'+ guid +'" role="button"/></div>');

	$('#closeNotaId_'+ guid +'').on("click keydown", function(e){
		if (e.type == "keydown" && !(e.keyCode == 13 || e.keyCode == 32)) return;
		accionCerrarNota(guid);
	});

	$('#deleteNotaId_'+ guid +'').on("click keydown", function(e){
		if (e.type == "keydown" && !(e.keyCode == 13 || e.keyCode == 32)) return;
...
{
	$('#listadoNotas').append('<div class="nota" id="'+ guid +'" tabindex="0" style="background-color:'+ color +';" role="button"><h1>'+ titulo +'</h1></div>');
	$('#'+ guid +'').on("click keydown", function(e){
		if (e.type == "keydown" && !(e.keyCode == 13 || e.keyCode == 32)) return;
		accionNota(guid);
	});
}
...
{
	$('#seleccionNotaFormulario').append('<div id="notaFormulario"><section name="formulario" class="formularioNota"><form id="formularioNotaId" method="post" onsubmit="return accionSubmitFormulario(this)" role="form"><label class="labelForm">Nueva Nota</label><input id="titleNote" type="text" maxlength="25" class="inputTitle" value="" placeholder="Titulo de la nota" pattern="^([a-zA-Z0-9 _%+-\¡!\¿\?ªº,.:áéíóúÁÉÍÓÚÑñ\(\))]){1,25}$" aria-required="true" required></input><textArea id="messageNote" class="textAreaMessage" placeholder="Mensaje de la nota" pattern="^([a-zA-Z0-9 _%+-\¡!\¿\?ªº,.:áéíóúÁÉÍÓÚÑñ\(\))])*$" aria-required="true" required></textArea><input id="colorNote" type="color" class="colorNota" value="#d698f3"> Color de la Nota</input><input type="image" src="img/icono_save.png" alt="Guardar Nota" class="inputSaveNota" id="saveNotaId" tabindex="0" onclick="" role="button"/></form>');
}
